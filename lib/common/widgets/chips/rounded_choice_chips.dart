    ...
    return Theme(
      ...
      child: ChoiceChip(
        ...
        avatar: MHelperFunctions.getColor(text) != null
            ? MCircularContainer(width: 50, height: 50, backgroundColor: MHelperFunctions.getColor(text)!)
            : null,
        selected: selected,
        onSelected: onSelected,
        backgroundColor: MHelperFunctions.getColor(text),
        labelStyle: TextStyle(color: selected ? MColors.white : null),
        shape: MHelperFunctions.getColor(text) != null ? const CircleBorder() : null,
        label: MHelperFunctions.getColor(text) == null ? Text(text) : const SizedBox(),
        padding: MHelperFunctions.getColor(text) != null ? const EdgeInsets.all(0) : null,
        labelPadding: MHelperFunctions.getColor(text) != null ? const EdgeInsets.all(0) : null,
      ),
    );
  }
}
